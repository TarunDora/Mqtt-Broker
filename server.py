...
# Function to notify subscribers of new messages for the subscribed topic
def notify_subscribers(topic, message):
    for subscriber_socket in active_subscribers:
        try:
            # Check if the subscriber is subscribed to the topic
            if authenticated_sessions[subscriber_socket] == topic:
                subscriber_socket.sendall(message.encode())
        except Exception as e:
            print(f"Error occurred while sending message to subscriber: {e}")
...
def handle_subscriber(client_socket, username):
    ...
    active_subscribers.append(client_socket)
        ...
        try:
            ...
            if command == "SUBSCRIBE":
                if content == "LIST_TOPICS":
                    ...
                    authenticated_sessions[client_socket] = content
                    client_socket.sendall("SUBACK".encode())

                    # Send existing messages for the topic
                    if content in topic_messages:
                        messages = "\n".join(topic_messages[content])
    ...
    # Remove subscriber from active list and close subscriber socket
    active_subscribers.remove(client_socket)
    client_socket.close()
...
def start_ssl_server():
    try:
        ...
        server_socket.bind(("127.0.0.1", 12559))
